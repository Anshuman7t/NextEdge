...
latin_countries = ['Anguilla', 'Antigua', 'Argentina', 'Aruba', 'Bahamas',
                  'Barbados', 'Belize', 'Bermudas', 'Bolivia', 'Brazil', 'British-Virgin-Islands',
                  ...
                  'Saint-Lucia', 'Saint-Vincent', 'Suriname', 'Trinidad',
                  'Turks-and-Caicos', 'US-Virgin-Islands', 'Uruguay', 'Venezuela']
...
# Convert CM to feet and inches (enhanced for robustness)
def cm_to_feet_inches(cm_text):
    if not cm_text:
        return None
    
    cm_text_clean = cm_text.strip().lower()

    # Try to extract a numeric value that could be CM (e.g., "201", "201cm")
    cm_value_match = re.search(r'^(\d+)(?:\s*cm)?$', cm_text_clean)
    if cm_value_match:
        try:
            ...
            # Not a valid number for CM
            pass
    
    # If already in feet/inches format, return as is (ensure consistent ' for inches)
    if "'" in cm_text_clean:
        return cm_text_clean.replace('"', "''").replace("''", "'") # Normalize single to double prime
...
def extract_player_page_details(player_url):
    ...
    if faq_section:
        ...
        for question_elem in questions:
            ...
            if answer_elem:
                ...
                # Extract Weight (store just the number)
                elif 'weight' in question_text:
                    weight_match = re.search(r'(\d+)\s*(?:kg|lbs)', answer_text, re.IGNORECASE)
                    if weight_match:
                ...
                # Extract Height (for refinement if roster height is basic)
                    ...
                    height_match_cm_ft = re.search(r'(\d+cm)\s*/\s*(\d+\'?\d*"?\s*(?:tall)?)', answer_text, re.IGNORECASE)
                    if height_match_cm_ft:
                        detailed_data['height_detail'] = height_match_cm_ft.group(2).replace('tall', '').strip()
                    else:
                        ...
                        else: # Try to get just CM and convert if no ft/in is found
                            height_cm_only_match = re.search(r'(\d+)\s*cm', answer_text, re.IGNORECASE)
                            if height_cm_only_match:
                                detailed_data['height_detail'] = cm_to_feet_inches(height_cm_only_match.group(1))
                    ...
                    for pattern in position_patterns:
                        ...
                        if pos_match:
                            ...
                            position_map = {
                                ...
                                'g/f': 'Swingman', 'f/g': 'Swingman', 'f/c': 'Forward/Center', 'c/f': 'Center/Forward' # Added for G/F to Swingman, F/C, C/F
                            }
    ...
    if not detailed_data['weight']:
        ...
        if weight_elem:
            ...
            weight_match = re.search(r'Weight:\s*(\d+)\s*(?:kg|lbs)', weight_text, re.IGNORECASE)
            if weight_match:
                detailed_data['weight'] = weight_match.group(1) # Store just the number
...
def get_roster(team_url, team_name, website, gender):
    ...
    for row in roster_rows:
        try:
            ...
            # Extract Height from roster (always attempt to convert to feet and inches)
            height_roster_cm_elem = cols[3].find('font', size="3")
            height_roster = None
            if height_roster_cm_elem:
                height_roster_text = height_roster_cm_elem.text.strip()
                height_roster = cm_to_feet_inches(height_roster_text) # Always try to convert
                logger.debug(f"Initial roster height '{height_roster_text}' converted to '{height_roster}'")
            ...
            if country_elem and 'alt' in country_elem.attrs:
                ...
                nationality_map = {
                    ...
                    'nigerian': 'Nigeria', 'usa-nigeria': 'Usa-Nigeria', 
                    'us': 'USA' 
                }
    ...
    for team in teams:
        ...
        for player_data in players_from_roster:
            if player_data['player_url']:
                ...
                # Now, both roster and detailed heights should be in feet/inches
                if detailed_player_info['height_detail']: # If detailed height is found, prefer it
                    player_data['height'] = detailed_player_info['height_detail']
                # Else, player_data['height'] already has the roster height in feet/inches from get_roster
                
                # Prioritize detailed position if it's more descriptive or the roster one was abbreviated
                if detailed_player_info['position_detail'] and (not player_data['position'] or len(player_data['position']) < 3):
                ...
                if player_data['position']:
                    position_map = {
                        ...
                        'c': 'Center',
                        'g/f': 'Swingman', 'f/g': 'Swingman', 'f/c': 'Forward/Center', 'c/f': 'Center/Forward' # Added for G/F and F/G to Swingman, F/C, C/F
                    }
...
async def start_scraping():
    ...
    for country in latin_countries: # Changed to latin_countries
        for gender in ['men', 'women']:
            base_url = 'https://basketball.latinbasket.com' # Changed base_url
            try:
                process_country(country, gender, base_url, 'basketball.latinbasket.com') # Changed website source
            except Exception as e:
