...
# List of African countries to scrape
african_countries = ['Algeria', 'Angola', 'Benin', 'Botswana', 'Burkina Faso',
                     'Burundi', 'Cameroon', 'Cape Verde', 'Central-Africa','Chad','Congo-Brazaville',
                     'DR Congo', 'Egypt', 'Ivory Coast',
...
def insert_player(data):
    try:
        ...
        conn.commit()
        logger.info(f"Successfully inserted players: {data['name']}")
        return True
    except psycopg2.Error as e:
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:  # Single worker for debugging
        ...
        for country in african_countries:
            for gender in ['men', 'women']:
                base_url = 'https://www.afrobasket.com'
                process_country(country, gender, base_url, 'basketball.afrobasket.com')
